...
> ⚠️ **STAGED — describes v6.2.0-era capabilities (Forge/Anvil, `Skill("ISA")`, 12-section ISA); NOT fully the active doctrine.** `Algorithm/LATEST` = v5.7.2 (which writes the ISA directly, no `Skill("ISA")`). Treat v6.2.0-specific mandates here as staged-ahead, not current. *(Spine P2 stamp, 2026-05-30.)*
...
| **MultiEngineSynthesis** | **BUILD/EXECUTE, VERIFY** | **Cross-engine consolidation: multiple independent assessments/reviews/research on the same topic need merging into one authoritative output. Triggers when 2+ engine artifacts exist or parallel agents spawned.** | `Skill("MultiEngineSynthesis")` | **E3+** |
...
| SecurityTriage | OBSERVE, THINK, VERIFY | Audit security posture, credential hygiene, or verify codebase interfaces for secrets. Triggers discovery-verification-ranking loop with 3 lens-diverse voters. | `Skill("SecurityTriage")` | E3+ |
...
| **pai-security-report** | Pre-push/on-demand security posture and secret scanning | `Shell: pai-security-report` |
| **pai-security-doctor** | Diagnose and verify integrity of security gates/hooks | `Shell: pai-security-doctor` |
