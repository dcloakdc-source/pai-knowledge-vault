...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "MESSAGE", "voice_id": "af_sky", "voice_enabled": true}'
```
...
**Voice:** `curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d '{"message": "Entering the Algorithm", "voice_id": "af_sky", "voice_enabled": true}'`
...
- **SELF-INTERROGATION** (scales by effort level):
  **Standard:** Answer Q1 and Q4 only, one line each.
  **Extended+:** Answer all 5 questions:
  1. "Is there anything in this request I have NOT captured — constraints, rules, thresholds, prohibitions?"
  2. "Are there specific numbers, limits, or quantitative bounds I must preserve verbatim?"
  3. "Are there explicit prohibitions ('don't', 'never', 'avoid', 'must not') not listed?"
  ...
  [List gaps found → add to explicit/implied lists above before proceeding.]

- **CONSTRAINT EXTRACTION** (scales by effort level):

  **Purpose:** Extract every rule, threshold, prohibition, and requirement from source material. Prevents the abstraction gap where specific constraints become vague ISC.

  - **Standard:** Compact numbered list: `EX-1: [constraint]. EX-2: [constraint].` No scanning protocol.
  - **Extended+:** Full 4-scan protocol:
    ...
    **SCAN 4 — Implicit** (conventions, patterns, domain norms):
    [EX-N: {inferred constraint with reasoning}]

    **Constraint Count:** [Total: N | Quantitative: X | Prohibitions: Y | Requirements: Z | Implicit: W]

  🚫 **SPECIFICITY PRESERVATION RULE:** NEVER paraphrase numbers, thresholds, or specific values. Copy verbatim.

  🔒 **CONSTRAINT EXTRACTION GATE (Extended+ only):**
    [N constraints extracted] → proceed | [0 constraints at Extended+] → **BLOCKED.** Re-scan source material.
...
- **Confidence Tags:** Tag each criterion: `[E]`=Explicit (user stated), `[I]`=Inferred (implied by context), `[R]`=Reverse-engineered (intuited ideal state). THINK phase focuses pressure testing on `[I]` and `[R]` criteria.
- **Priority Classification (Extended+ only):** Tag each criterion with priority:
  - `[CRITICAL]` = From explicit constraint [EX-N] or prohibition. Violation = task failure. Gets Verification Rehearsal + Build Drift checks.
  - `[IMPORTANT]` = From inferred requirement. Violation = significant quality issue.
  - `[NICE]` = From reverse-engineered ideal state. Violation = missed opportunity.
...
For each extracted constraint [EX-N], state which ISC criterion covers it:
  EX-1 → ISC-{N} | EX-2 → ISC-{M} | ...
**UNMAPPED CONSTRAINTS = BLOCKED.** Every [EX-N] must map to at least one ISC. If unmapped, create additional criteria before proceeding.

🔒 **ISC QUALITY GATE (MANDATORY — cannot proceed to THINK without passing):**

QG1 — Count the criteria. Compare against effort tier minimum:
...
**If ISC count < floor: DO NOT proceed.** Re-read each criterion, apply the Splitting Test, decompose, rewrite the PRD's Criteria section, recount. Repeat until floor is met. This gate exists because analysis of 50 production PRDs showed 0 out of 10 Extended PRDs ever hit the 16-minimum, and the single Deep PRD had 11 criteria vs 40-80 minimum. The gate is the fix.
  ...
  QG6 Coverage (Extended+ only):    [PASS: every [EX-N] in Coverage Map maps to ≥1 ISC] or [FAIL: EX-{N} unmapped]
  QG7 Specificity (Extended+ only): [PASS: no criterion abstracts number/threshold into vague qualifier] or [FAIL: ISC-C{N} abstracts EX-{M}]
...
For each [CRITICAL] ISC criterion:
  1. **Simulate violation:** What would a concrete failure look like?
  2. **Test detection:** Would VERIFY actually catch this violation?
  3. **Fix gap:** If undetectable, strengthen the verification method now.
[If no [CRITICAL] criteria: confirm all constraints covered by [IMPORTANT] criteria.]

🔍 **SELF-INTERROGATION CHECK:** "What is the most likely criterion I will accidentally violate during BUILD?" Name it explicitly.

🔍 **CONFIDENCE TAG REVIEW:** Focus pressure testing on `[I]` (Inferred) and `[R]` (Reverse-engineered) criteria — these are most likely to be wrong or incomplete.
...
Before creating artifacts, re-read all [CRITICAL] ISC criteria and anti-criteria:
  "I am about to create [artifact]. My [CRITICAL] criteria are: [list]. My [CRITICAL] anti-criteria are: [list]."
  Standard: state criteria once at BUILD start. Extended+: re-state before EACH artifact.
...
🔍 **CONSTRAINT CHECKPOINT (after creating artifacts):**
After creating each artifact, check all [CRITICAL] anti-criteria against what was built:
  For each [CRITICAL] anti-criterion: "Does this artifact violate [anti-criterion]? Evidence: [specific check]."
  If ANY violation found → fix BEFORE creating next artifact. Do NOT batch to VERIFY.
  Standard: checkpoint once after all artifacts. Extended+: after EACH artifact.
...
- **ISC Quality Gate is mandatory** — Cannot exit OBSERVE without passing QG1-QG7. QG1 enforces count floors (Standard: 8, Extended: 16, Advanced: 24, Deep: 40, Comprehensive: 64). QG2-QG5 enforce quality at all tiers. QG6-QG7 enforce constraint coverage at Extended+. If any gate fails, fix before proceeding.
- **Confidence tags mandatory on every criterion** — Every ISC must have `[E]`, `[I]`, or `[R]` tag. No untagged criteria.
