...
> **v5.7.12 changes from v5.7.11** (2026-07-06, six-source upgrade review `20260706-022701_pai-upgrade-source-review` + reflection mining, applied with explicit consent — Duane: "address all". NOTE: this version number was previously used by a same-day-reverted ISC-N.M/tombstone proposal (commit 990786d5); THIS v5.7.12 is unrelated content and supersedes nothing from that revert):
> - **COORDINATION GATE updated for harness reality** — `TeamCreate`/`TeamDelete` were REMOVED in Claude Code v2.1.178; every session has one implicit team and teammates spawn directly via the `Agent` tool's `name` parameter (`team_name` is accepted-but-ignored). The gate text no longer instructs a tool that does not exist. Also: subagents run in the BACKGROUND BY DEFAULT since v2.1.198 — background use is ambient, not an opt-in; `agent_completed`/`agent_needs_input` Notification events drive voice (AgentEventVoice.hook.ts).
> - **Rule 2 advisor fact-manifest mandate** — advisor invocations MUST pass a session-probed fact manifest as manual STATE (file:line quotes, command outputs verified this session); `--auto-state` alone is FORBIDDEN for PLAN→BUILD gates. Closes the dominant fresh reflection theme (freq 6, three occurrences on 2026-07-05 alone): auto-state reads stale/permission-gated filesystem state and asserts phantom claims ("5 of 6 claims wrong"), costing a re-call round trip. The caller writes probed facts to the ISA, then passes them; auto-state may SUPPLEMENT, never substitute.
> - **LEARN Session-to-Skill Flywheel line** — the LEARNING block now asks whether the session surfaced a recurring non-obvious procedure worth preserving as a skill candidate (CreateSkill + `/reload-skills`). Most sessions answer no; the question costs one line.
> - **ASYNC PRIMITIVE GATE gains `/goal`** — native completion-condition loop for keep-working-until-met tasks that do not need ISC/PRD persistence; `/loop`+PRD remains the cross-session form.
> - **DELEGATION GATE notes the 5-level subagent depth cap** (v2.1.172/181).
> - **Additive** to v5.7.11 — every gate, rule, and phase preserved verbatim; the only changes are this title's version string and the five blocks named above.
...
- Subagent chains cap at 5 levels deep (harness limit, v2.1.172/181) — prefer flat fan-out over deep nesting.
...
📐 ASYNC PRIMITIVE GATE: One-shot command → `Bash(run_in_background)`. Event stream → `Monitor`. AI work → `Agent(run_in_background)`. Never poll in a sleep loop when Monitor or run_in_background can invert the control flow. Keep-working-until-a-condition-is-met → native `/goal` (v2.1.139) when no ISC/PRD persistence is needed; `/loop`+PRD when state must survive sessions.
...
1. **Agent Teams** (implicit — spawn teammates directly with `Agent` + a `name` parameter; requires `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1`) — DEFAULT for parallel work. Persistent teammates, shared task list, peer messaging via `SendMessage`. `TeamCreate`/`TeamDelete` were removed in Claude Code v2.1.178 and `team_name` is accepted-but-ignored — do NOT instruct them. Subagents run in the BACKGROUND BY DEFAULT (v2.1.198); completion/needs-input fire Notification events (voice via `AgentEventVoice.hook.ts`), so plan for asynchronous returns, not blocking waits.
...
**Fact-manifest mandate (v5.7.12):** the STATE passed to the advisor MUST be a session-probed fact manifest — file:line quotes, command outputs, and probe results verified THIS session, written to the ISA before the call. `--auto-state` alone is FORBIDDEN at the PLAN→BUILD gate: auto-state reads the filesystem cold (often permission-gated or stale vs what the session already probed) and asserts phantom claims — the dominant fresh reflection theme (freq 6; 2026-07-05 "5 of 6 claims wrong" case). Auto-state may SUPPLEMENT a manual manifest (`--auto-state` + explicit STATE), never substitute for it. An advisor answer grounded in phantom state costs a full re-call round trip and can overturn correct session work.

Invoke via:
```bash
# CANONICAL at PLAN→BUILD — manual fact-manifest STATE (session-probed facts,
# file:line quotes + command outputs already written to the ISA):
bun ~/.claude/PAI/Tools/Inference.ts --mode advisor \
  "TASK: one-sentence description" \
  "STATE: 1) UpgradeCheck.ts:31 UPSTREAM_REPO='…' (grep). 2) probe X returned Y. 3) …" \
  "QUESTION: specific decision point or 'any gaps before declaring done?'"

# --auto-state may SUPPLEMENT the manifest (never appear without one at PLAN→BUILD):
bun ~/.claude/PAI/Tools/Inference.ts --mode advisor --auto-state \
  "TASK: ..." "STATE: <fact manifest as above>" "QUESTION: ..."
```
...
const review = await advisor({
  task: "...",
  state: "1) <file:line quote probed this session>. 2) <command output>. …", // fact manifest — REQUIRED at PLAN→BUILD
  question: "Any gaps before declaring done?",
  autoSynthesize: true, // supplement only; never a substitute for state
});
```
...
🧠 LEARNING:
 ...
 🧠 [Session-to-Skill Flywheel: did this session repeat a non-obvious procedure worth preserving? Usually NO — but a yes becomes a CreateSkill candidate + `/reload-skills`, not a memory note.]
```
