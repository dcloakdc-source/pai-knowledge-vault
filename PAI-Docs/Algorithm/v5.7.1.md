...
**Skip is permitted only if:**
- Run was explicitly classified as exploratory in `## Decisions` at OBSERVE, AND
- Fast-path mode (Standard tier, explicit fast-path), OR
- **Measured-duration check** (v3.24 P2 + v5.7.1 hardening): actual wall-clock work stayed under 4 minutes AND touched fewer than 2 files AND PLAN produced fewer than 2 Deliverable Manifest entries AND ISC count stayed below the E2 floor (16). If ANY threshold is exceeded, the advisor call becomes MANDATORY regardless of initial classification.

**Why the AND chain is hardened:** v5.7.0's wall-clock + file-count check missed cases where PLAN already named 3 deliverables but the work happened to fit inside 4 minutes. Manifest size and ISC count are PLAN-phase signals — they predict commitment scope before the BUILD work begins, which is the right input for a PLAN→BUILD gate. *(provenance: v5.7.1.)*
