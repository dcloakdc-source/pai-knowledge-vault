...
### 8. **StopFailure** *(v2.1.79+)*
**When:** Claude Code fails to complete a response due to a terminal error (rate limit, auth failure, network timeout, API overload)
**Status:** Active — `hooks/StopFailure.hook.ts`
...
```typescript
{
  ...
  stop_reason?: string;  // e.g. "rate_limit_exceeded", "auth_failure", "overloaded_error"
  error?: string;        // Human-readable error description
}
```

**What it does:**
- Classifies failure as: `rate_limit` | `auth_failure` | `overload` | `network` | `unknown`
- Fires voice notification via voice server (`http://localhost:8888/notify`)
- Appends structured event to `MEMORY/LEARNING/failures.jsonl`

**Why it matters:** Silent failures are invisible — especially problematic for ADHD workflows. StopFailure turns every session interruption into an observable, logged event (PAI externalization strategy S3).

---

### ⚠️ Deprecated: `resume` param in Agent tool calls *(removed v2.1.77)*
The `resume` parameter that was previously available in `Agent` tool calls has been **removed** in Claude Code v2.1.77. Using it will cause an error.

**Before (broken):**
```json
{ "subagent_type": "general-purpose", "prompt": "...", "resume": true }
```

**After (correct):** Use `SendMessage` with the agent's `name` or `id` to continue a named agent:
```json
// First call — give the agent a name:
{ "subagent_type": "general-purpose", "name": "my-researcher", "prompt": "Start researching X" }
// Continue the agent — use SendMessage:
// SendMessage({ to: "my-researcher", message: "Now summarize what you found" })
```
...
**Fields:**
- `HookEventName` - One of: SessionStart, SessionEnd, UserPromptSubmit, Stop, StopFailure, PreToolUse, PostToolUse, PreCompact, SubagentStop, PostCompact, ConfigChange, Elicitation, ElicitationResult
